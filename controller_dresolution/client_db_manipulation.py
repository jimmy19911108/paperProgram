...
class ClientDBManipulation():
    ...
    def get_all_client(self, turn_addr):
        '''
        get all clients' id which are connecting to the turn
        '''

        self.client_curs.execute(
            '''SELECT * from client where turn_addr = ?''', (turn_addr,))

        return self.client_curs.fetchall()