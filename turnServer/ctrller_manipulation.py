...
class CtrllerManipulation():
    ...
    def socket_io(self):
        ...
        try:
            while True:
                if (time.time() - old_time) >= 180 or \
                   int(self.database.get_client_cumulate().decode()) > 0:

                    all_usage = self.res_usage.get_all_info()
                    self.send_data(
                        "turn " + str(all_usage[0]) + " " + str(all_usage[1]))
                    self.database.reset_connect_client_cumulate()
                    old_time = time.time()
